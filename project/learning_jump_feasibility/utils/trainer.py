...
class TrainerBase():
    ...
    def _update_lr(self):
        ...
        if self.scheduler:
            self.scheduler.step()
            current_lr = torch.tensor(self.scheduler.get_last_lr())
    ...
    def train(self):
        ...
        for epoch in progress_bar:
            ...
            if (self.dataloader_test != None and
                ...
                if (self.ckpt_every > 0 and 
                    epoch % self.ckpt_every == 0 and
                    epoch > 0):
                    self._save_model()
...
class TrainerDDPM(TrainerSupervised):
    ...
    def __init__(self,
        ...
        if not isinstance(model, DDPM) or not isinstance(model, CDCD):
            ddpm_model = DDPM(model, **cfg.training)
        else:
            ddpm_model = model
    ...
    def _test_epoch(self):
        ...
        for batch in tqdm(self.dataloader_test, desc = "Batch", leave=False):
            ...
            data_generated, _, _ = self.model.sample(condition=condition)
...
class TrainerCDCD(TrainerDDPM):
    ...
    def __init__(self,
        ...
        if not isinstance(model, CDCD):
            cdcd_model = CDCD(model, **cfg.training)
        else:
            cdcd_model = model
